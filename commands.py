...
def enterdungeon(username, message, channel):
    ...
    if user is not None and user.get('user_level') is not None:
        ...
        if user['entered'] == 0:
            ...
            db(opt.USERS).update_one(id, {'$set': {
                'entered': 1,
...
def dungeonmaster(channel):
    ...
    if topuser and topuser.get('user_level'):
        ...
        if numberoftopusers == 1:
            ...
            util.sendmessage(messages.dungeon_master(util.checkusername(thetopuser['_id']), str(highestexperience), str(userlevel)), channel)
        else:
...
def register(username, channel):
    ...
    if user == None or user.get('user_level') == None:
        ...
        db(opt.USERS).update_one(id, { '$set': schemes.USER }, upsert=True)
        dungeon = db(opt.GENERAL).find_one_by_id(0)
...
def userexperience(username, channel, message=None):
    if message is None:
        ...
        if message.lower() == username.lower():
            ...
            id = util.checkuserid(message)
            user = db(opt.USERS).find_one_by_id(id)
            if not util.checkuserid(message):
                registered = util.checkuserregistered(username, channel)
                if registered:
                    id = util.checkuserid(username)
                    user = db(opt.USERS).find_one_by_id(id)
                ...
                util.sendmessage(messages.user_experience(util.checkusername(user['_id']), str(user['total_experience'])), channel)
            else:
    ...
    if message is None:
        ...
        if message.lower() == username.lower():
            ...
            if not util.checkuserid(message):
                ...
                util.sendmessage(messages.user_level(util.checkusername(user['_id']), str(user['user_level']), str(user['current_experience']), str((((user['user_level']) + 1)**2)*100)), channel)
            else:
...
def winrate(username, channel, message=None):
    if message is None:
        ...
        if (message.lower() == username.lower()) or not util.checkuserid(message):
            registered = util.checkuserregistered(username, channel)
            if registered:
                id = util.checkuserid(username)
                user = db(opt.USERS).find_one_by_id(id)
            ...
            registered = util.checkuserregistered(username, channel, message)
            if registered:
                id = util.checkuserid(message)
                user = db(opt.USERS).find_one_by_id(id)
                if user is not None and user['dungeons'] is 0:
                    ...
                    if losses == 1:
                        ...
                        loseword = ' Losses'
                    util.sendmessage(messages.user_stats(util.checkusername(user['_id']), str(wins), winword, str(losses), loseword, str(round((((wins)/(dungeons))*100), 3))), channel)
